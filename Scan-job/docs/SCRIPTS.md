...
| `npm run clean-pipeline` | `clean-pipeline.mjs` | Normalize and deduplicate `pipeline.md` |
...
Normalizes `data/pipeline.md` to the canonical layout, removes duplicate pending entries, drops pending jobs that are already tracked or already processed, and sanitizes embedded `|` characters inside company/title fields so downstream parsing stays stable.

```bash
npm run clean-pipeline
npm run clean-pipeline -- --dry-run
```

Creates `data/pipeline.md.bak` before writing.
